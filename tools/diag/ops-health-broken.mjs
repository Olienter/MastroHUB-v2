...
async function main() {
  ...
  // pnpm and npm (using new helpers with env hints)
  const pnpmResult = await runPnpm(["-v"], { timeoutMs: 10000 }, report.env);
  nodeTools.pnpm = {
  ...
  const npmResult = await runNpm(["-v"], { timeoutMs: 10000 }, report.env);
  nodeTools.npm = {
